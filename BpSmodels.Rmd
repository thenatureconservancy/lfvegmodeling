...
**[LANDFIRE BpS models](https://www.landfire.gov/bps-models.php)** are **reference condition models** that define natural vegetation communities and their historical disturbance regimes. 
</div>
<br>

**[LANDFIRE Biophysical Settings](https://www.landfire.gov/bps.php)** represent vegetation that may have been dominant on the landscape just prior to Euro-American settlement. They are based on [NatureServe's Ecological Systems](https://www.natureserve.org/conservation-tools/terrestrial-ecological-systems-united-states). 
**Biophysical Settings** are mapped (for more information on the spatial data visit [this site](https://www.landfire.gov/bps.php)), described below, and modeled using SyncroSim.  
...
```{r, echo=FALSE, fig.cap = "Photo: © Ian Shive, TNC, \n Independence Lake, Nevada", out.width='65%', out.extra='style="float:right; padding:10px"'} 
...
1. **Classify and define the ecosystems.** LANDFIRE calls the historic ecosystems "Biophysical Settings", and used [NatureServe's Ecological Systems](https://www.natureserve.org/conservation-tools/terrestrial-ecological-systems-united-states)

2. The LANDFIRE team held expert workshops to:
    * **Describe the BpSs**, including their nested **[succession classes](https://www.landfire.gov/sclass.php),** up to 5 for each BpS including their canopy height, composition and percent cover. 
...
#### Descriptions
Each BpS has an [extensive description](http://landfirereview.org/search.php) See the description from the [Montane Sagebrush Steppe Ecosystem](http://landfirereview.org/search.php?q=Montane%20Sage%20Steppe&hPP=20&idx=lf_landfire_dev&p=0&is_v=1) example. Included in each description:
...
Importantly, these models and descriptions were designed for use over large areas - not your back yard. BpS models were developed for use at the [LANDFIRE Map Zone](http://landfirereview.org/IMG/USmapzones-v1_July2015.jpg) level (hundreds of thousands of acres, see "Get Models" page). That said, people do use them for smaller areas.  Success of downscaling depends on factors such as:
